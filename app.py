    ...
    for idx, result in enumerate(st.session_state["ocr_result"]):
        ...
        if file_type=="PDF":
            ...
            if source_type=="Local Upload" and st.session_state["image_bytes"]:
                st.image(st.session_state["image_bytes"][idx], caption=f"Image")
            else:
        ...
        if download_choice=="TXT":
            st.download_button(" Download TXT", result, file_name=f"OCR_Result.txt")
        elif download_choice=="Markdown":
            st.download_button(" Download MD", result, file_name=f"OCR_Result.md")
        elif download_choice=="JSON":
            json_data = json.dumps({"ocr_result": result}, ensure_ascii=False, indent=2)
            st.download_button(" Download JSON", json_data, file_name=f"OCR_Result.json")
