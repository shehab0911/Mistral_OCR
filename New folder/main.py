...
with st.sidebar:
    ...
    # Show file type & source type only if API key entered
    if api_key.strip():
...
# Upload or URL input (from sidebar selection)
uploaded_files = []
input_url = ""
...
if st.session_state.get("ocr_result"):
    for idx, result in enumerate(st.session_state["ocr_result"]):
        ...
        # Download options (TXT, JSON, Markdown, PDF)
        st.write("### 💾 Download Options")
        def create_download_link(data,filetype,filename):
            b64 = base64.b64encode(data.encode()).decode()
            href = f'<a href="data:{filetype};base64,{b64}" download="{filename}">⬇️ {filename}</a>'
            st.markdown(href, unsafe_allow_html=True)

        json_data=json.dumps({"ocr_result":result}, ensure_ascii=False, indent=2)
        create_download_link(json_data,"application/json",f"OCR_Result_{idx+1}.json")
        create_download_link(result,"text/plain",f"OCR_Result_{idx+1}.txt")
        create_download_link(result,"text/markdown",f"OCR_Result_{idx+1}.md")

        # PDF download (mandatory)
        pdf_buffer = BytesIO()
        ...
        href_pdf = f'<a href="data:application/pdf;base64,{b64_pdf}" download="OCR_Result_{idx+1}.pdf">📘 Download PDF</a>'
        st.markdown(href_pdf, unsafe_allow_html=True)

        # Extracted Text Box
        #st.markdown("<pre>"+result+"</pre>", unsafe_allow_html=True)
